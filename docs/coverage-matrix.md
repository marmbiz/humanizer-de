...
Status: P0 source-of-truth matrix for v5.24.0.
...
| Two-pass runner | `scripts/humanizer_two_pass.py` | Unicode-normalized intake, separate read-only audit, fresh candidate-bound rewrite with a frozen protection ledger, and original-to-output change verification | Untouched original, normalized downstream input, host-enforced spans, structural scope, delete-only candidates, anchor checks, Evidence gate, unified `changes.diff`, and full `verify.json` artifacts | Complete fact/source verification or proof that every model-selected candidate is correct |
